...
void xdpw_screencast_instance_init(struct xdpw_screencast_context *ctx,
                                   struct xdpw_screencast_instance *cast, struct xdpw_share out) {
...
bool setup_target(struct xdpw_screencast_context *ctx, struct xdpw_session *sess, struct xdpw_screencast_restore_data *data) {
	...
	*/
	...
	if (!sess->screencast_data.screencast_instance) {
		...
		xdpw_screencast_instance_init(ctx, sess->screencast_data.screencast_instance, out);
	}
    ...
    if (out.output) {
        ...
        logprint(INFO, "hyprland: window handle %d", sess->screencast_data.screencast_instance->target.window_handle);
    }
	return true;
}
